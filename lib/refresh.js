...
const { asSlug } = pageModule;
const { pageEmitter } = pageModule;
...
const handleDrop = function (evt, ui, originalIndex, originalOrder) {
  ...
  const sourceIsReadOnly =
    $sourcePage.hasClass("ghost") || $sourcePage.hasClass("remote");
  ...
  pageHandler.put($destinationPage, {
    ...
    type: "add",
    item,
    after: before != null ? before.id : undefined,
...
const changeMouseCursor = function (e, ui) {
  ...
  const moveWithinPage = equals($sourcePage, $destinationPage);
  const moveBetweenDuplicatePages =
    !moveWithinPage && $sourcePage.attr("id") === $destinationPage.attr("id");
...
const handleMerging = function (event, ui) {
  ...
  pageEmitter.dispatchEvent(
    new CustomEvent("show", { detail: drop.merge(drag) }),
  );
...
const initAddButton = ($page) =>
  $page.find(".add-factory").on("click", function (evt) {
...
var createFactory = function ($page) {
  ...
  const $item = $("<div />", { class: "item factory" })
    .data("item", item)
...
const handleHeaderClick = function (e) {
  ...
  const [target] = Array.from(crumbs);
  let [prefix] = Array.from(wiki.site(target).getDirectURL("").split("/"));
  if (prefix === "") {
  ...
  if (Object.keys(backlinks).length > 0) {
    ...
    for (var linkSlug in backlinks) {
      ...
      for (var i = 0; i < backlink.sites.length; i++) {
        ...
        if (i < 10) {
          var joint =
            __guard__(backlink.sites[i - 1], (x) => x.date) === site.date
              ? ""
              : " ";
          flags.unshift(joint);
          flags.unshift(`\
  ...
  if ((viewing = editDate(page.journal))) {
    ...
    for (remoteSite in neighborhood.sites) {
      ...
      if (remoteSite !== site && info.sitemap != null) {
        for (item of Array.from(info.sitemap)) {
          if (item.slug === slug) {
            bin =
              item.date > viewing
                ? bins.newer
...
const renderPageIntoPageElement = function (pageObject, $page) {
  ...
  const $pagehandle = $("<div />")
    .addClass("page-handle")
    .appendTo($handleParent);
...
const createMissingFlag = function ($page, pageObject) {
  if (!pageObject.isRemote()) {
    return $("img.favicon", $page).on("error", () =>
      plugin.get("favicon", (favicon) => favicon.create()),
    );
  }
};
...
const newFuturePage = function (title, create) {
  ...
  if (hits.length > 0) {
    pageObject.addItem({
      ...
      title: title,
      create: create,
      context: pageHandler.context.filter(
        (c) => !["view", "origin", "local"].includes(c),
      ),
    });
    pageObject.addItem({
      type: "paragraph",
    ...
    for (var hit of Array.from(hits)) {
...
const cycle = function ($page) {
  var promise = new Promise(function (resolve, _reject) {
    const [slug, rev] = Array.from($page.attr("id").split("_rev"));
    ...
    const whenNotGotten = function () {
      ...
      const create = __guard__(lineup.atKey(key), (x) => x.getCreate());
      const pageObject = newFuturePage(title);
      promise = buildPage(pageObject, $page);
      promise.then(($page) => $page.addClass("ghost"));
      return resolve(promise);
    };

    const whenGotten = function (pageObject) {
      promise = buildPage(pageObject, $page);
      for (var site of Array.from(pageObject.getNeighbors(location.host))) {
