...
var newPage = function (json, site) {
  ...
  const getContext = function () {
    ...
    const addContext = function (site) {
      if (site != null && !_.include(context, site)) {
        return context.push(site);
      }
    };
    for (var action of Array.from(page.journal.slice(0).reverse())) {
      addContext(action != null ? action.site : undefined);
    }
    return context;
  };
  ...
  var isRemote = () =>
    ![undefined, null, "view", "origin", "local", "recycler"].includes(site);
  ...
  const getNeighbors = function (host) {
    ...
    for (var item of Array.from(page.story)) {
      if ((item != null ? item.site : undefined) != null) {
        neighbors.push(item.site);
      }
    }
    for (var action of Array.from(page.journal)) {
      if ((action != null ? action.site : undefined) != null) {
  ...
  const getLinks = function () {
    ...
    const extractPageLinks = function (
      collaborativeLinks,
      currentItem,
      currentIndex,
      array,
      ...
      try {
        ...
        console.log(
          `*** Error extracting links from ${currentIndex} of ${JSON.stringify(array)}`,
          err.message,
        );
      }
  ...
  const become = function (story, journal) {
    ...
    if (journal != null) {
      return (page.journal =
        journal != null ? journal.getRawPage().journal : undefined);
    }
  };
  ...
  const merge = function (update) {
    ...
    const merged = (() => {
      const result = [];
      for (action of Array.from(page.journal)) {
        result.push(action);
      }
      return result;
    })();
    ...
    return newPage(
      revision.create(999, { title: page.title, journal: merged }),
      site,
    );
  };
